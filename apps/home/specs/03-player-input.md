...
- If the hex is adjacent to the player's unit and the unit has movement budget remaining, the unit moves to that hex.
- If the hex is not adjacent, or the unit has no movement budget, no action is taken.
- *(Notable: multi-step pathfinding to a non-adjacent destination is out of scope for the initial implementation.)*
...
- When a unit is selected, valid move destinations are visually highlighted.
...
- Clicking a non-adjacent hex does not move the unit.
