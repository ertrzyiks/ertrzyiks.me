...
import { type GameTileHex, cubeToCartesian } from "../core";
...
export class EditorWorld extends Container {
  ...
  protected setupFolder: GUI | null = null;
  ...
  constructor(protected events: EventSystem) {
    super();
  ...
  setupTileFolder(point: PointLike) {
    ...
    if (!this.tileFolder) {
      ...
      this.tileFolder
        .add(this.game_data.tile, "textureName", ["water", "grass"])
        .onChange(this.onTextureNameChange.bind(this));
      this.tileFolder
        .add(this.game_data.tile, "sectionName", ["none", "spawn_a", "spawn_b"])
        .onChange(this.onSectionNameChange.bind(this));
    }
  ...
  onLevelSelect(name: string) {
    ...
    Api.get(name).then((data) => {
      ...
      settingsFolder
        .add(this.game_data, "rows", 1, 100, 1)
        .onChange(updateSize);
      settingsFolder
        .add(this.game_data, "columns", 1, 100, 1)
        .onChange(updateSize);
