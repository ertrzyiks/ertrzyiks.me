 ...
 * `extract-action-items`, `sync-todoist`, `sync-calendar-events`, `refresh-library-loans`, and
 * `sync-loan-calendar` — all five consumed by `Worker`s started right here in this process (see
 * server.ts), each only once its own credentials are set. No `readOnlyMode` is set on the
